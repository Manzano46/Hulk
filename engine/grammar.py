...
semi, comma, opar, cpar, arrow, colon, semi, opcur, clcur, opcor, clcor, dot, com = G.Terminals('; , ( ) -> : ; { } [ ] . "')
    ...
    # operators
equal, plus, minus, star, div, greater, less, mod, pot, or_, andp, not_, equal_equal, not_equal, greater_equal, less_equal, destructive, arrow, pow2, doble_or = G.Terminals('= + - * / > < % ^ | & ! == != >= <= := => ** ||')
...
expr_list %= global_expression, lambda h, s: [s[1]] #                               | expression;

body %= arrow + expr + semi, lambda h,s : s[3] # body -> => expression ;
...
assignment_exp %= boolean_exp + destructive + assignment_exp , lambda h,s : DestructiveAssignmentNode(s[1], s[4]) # assignment_expression -> boolean_expression := assignment_expression
assignment_exp %= boolean_exp, lambda h,s : s[1] #                                                                                         | boolean_exp

boolean_exp %= boolean_exp + or_  + comp_exp, lambda h,s : OrNode(s[1], s[4]) # boolean_expression -> boolean_expression | comparation_expression
boolean_exp %= boolean_exp + andp + comp_exp, lambda h,s : AndNode(s[1], s[4]) #                    |boolean_expression & comparation_expression
boolean_exp %= comp_exp, lambda h,s : s[1] #                                                        |comparation_expression

comp_exp %= comp_exp + equal_equal + is_as_exp, lambda h,s : EqualNode(s[1], s[4]) # comparation_expression -> comparation_expresion == comparation_expression
comp_exp %= comp_exp + not_equal + is_as_exp, lambda h,s : NotEqualNode(s[1], s[4]) #                        | comparation_expression != comparation_expression
...
comp_exp %= comp_exp + greater_equal + is_as_exp, lambda h,s : GreaterOrEqualNode(s[1], s[4]) #              | comparation_expression >= comparation_expression
comp_exp %= comp_exp + less_equal + is_as_exp,lambda h,s : LessOrEqualNode(s[1], s[4]) #                     | comparation_expression <= comparation_expression
...
pow_exp %= type_exp + pow2 + pow_exp, lambda h,s : PowNode(s[1], s[4]) # pow_expression -> pow_expression ** type_expression
...
vector_exp %= opcor + expr + doble_or + idx + in_ + expr + clcor, lambda h,s : VectorComprehensionNode(s[2],s[5],s[7]) # | [expression || id in expression]
...
assignment_list %= assignment + assignment_list, lambda h,s : [s[1]] + s[2] # | var = expression, var = expression ...
